...
description: Strategic research companion — brainstorm, evaluate, and decide on research directions. Use when the user wants to brainstorm research, evaluate research ideas, do project triage, or explore a problem space. Orchestrates brainstormer, idea-critic, and research-strategist agents through a 6-phase pipeline.
license: MIT
...
- **One Phase Per Turn:** Never execute multiple phases in a single response. Do not hallucinate or simulate the requested answers, which should be populated by subagents.
- **Subagent Delegation:** For Phases 2, 3, and 4, you must rely on your specialized subagents (`@brainstormer`, `@idea-critic`, `@research-strategist`). Do not attempt to do their deep-dive work yourself.
- **Mandatory Grounding:** Relying solely on internal knowledge for competitive analysis is strictly forbidden. Web search MUST be utilized when assessing competitive landscape.
...
* **`@idea-critic`**: Adversarially stress-tests ideas along 7 dimensions (Novelty, Impact, Timing, Feasibility, Competition, Nugget, Narrative).
* **`@research-strategist`**: Assesses scooping risk, comparative advantage, and timing.
...
If the user provided a clear and detailed description, you may skip directly to Phase 2.
...
Do not immediately implement this first step. We should be opening a dialogue with the user by presenting these concrete POSSIBLE first steps.
...
1. Create `research-evaluations/` directory if it doesn't exist.
2. Write evaluation file: `research-evaluations/YYYY-MM-DD-<topic-slug>.md` containing:
   ...
   ```
3. Confirm to the user: "Evaluation saved. I'll check for this next time you explore a similar topic."
